...
for _, direction in ipairs(directions) do
    ...
    local suffix = direction == "forward" and " (Forward)" or " (Backward)"
    local script_name

    if apply_to_all_plugins then
        script_name = "[Generated]_Selecting all plugins one by one in loop " .. track_name .. (fx_container > 0 and " " .. fx_container .. "Container" or "") .. suffix .. ".lua"
    else
        script_name = "[Generated]_Selecting plugins in range(" .. min_fx + 1 .. "-" .. (max_fx + 1) .. ") one by one in loop " .. track_name .. (fx_container > 0 and " " .. fx_container .. "Container" or "") .. suffix .. ".lua"
    end
